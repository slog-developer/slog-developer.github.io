/*!
 * manualBanner v10. ~ Copyright (c) 2013 Hangun's World, http://hangunsworld.com/blog/
...
/**
 * initializes manualBanners
 */
function initmanualBanner(){
	var wrap;
	var W;
	var H;
	var lis;
	...
	var type;
	var nav_type;
	$(".manual_banner_wrap").each(function(idx){
		// init variables
		wrap = $(this);
		wrap.data("index", idx);
		wrap.data("current", 0);
...
function sbAnimateBannerTimer(idx){
	var wrap = $(".manual_banner_wrap").eq(idx);
...
function sbNavClickListener(e){
	var n;
	var nav = $(e.target);
	var wrap = nav.parents(".manual_banner_wrap");
	switch(wrap.data("nav-type")){
		case "numeral":
		case "bullet":
			n = nav.html() - 1;
